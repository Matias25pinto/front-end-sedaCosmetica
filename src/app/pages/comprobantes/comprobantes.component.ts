...
export class ComprobantesComponent implements OnInit {
  ...
  private id;
  constructor(private arqueoService: ArqueoService, private route: ActivatedRoute, private router:Router, private usuarioService: UsuarioService) { }
  ...
  cargarComprobante() {
    ...
    this.route.params.subscribe(parametros => {
      ...
      console.log(this.id);
      this.arqueoService.getArqueo(loginToken, this.id).subscribe(data => {
        ...
        if (this.comprobantes.length > 0) {
          for (const elemento of this.comprobantes) {
            ...
            this.montoComprobante = this.montoComprobante + Number(elemento.monto);
          }
        }
  ...
  eliminarComprobante(id, indexEliminar) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.arqueoService.eliminarComprobante(loginToken, id, comprobantesUpdate).subscribe(data => {
          console.log(data);
          Swal.fire(
            'Eliminado!',
