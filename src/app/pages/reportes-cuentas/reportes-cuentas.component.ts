...
export class ReportesCuentasComponent implements OnInit {
  ...
  async generarPdf(informe) {
    ...
    for (const cuenta of cuentasbancos) {
      ...
      for (const comprobante of informe.comprobantesDeposito) {
        if (comprobante.cuentaBancaria === cuenta) {
          if (cuenta === 'S.R.L. | 900 580 747 | 80021496-0') {
            console.log(informe);
          }
    ...
    for (const banco of cuentasbancos) {
      let bancoReporteDepositoCuenta = new Txt(`CUENTA: ${banco} = ${this.number_format(cuentaMonto[indexCuentaMonto], 0)} Gs.`)
        .bold()
        .decorationStyle('dotted')
        .fontSize(12)
        .alignment('left').end;
    ...
    let tablaDeposito = new Table(datosTablaDepositoCuenta).relativePosition(70,3).alignment('center').layout('lightHorizontalLines').end;
