...
export class ArqueosComponent implements OnInit {
  ...
  constructor(private arqueoService: ArqueoService, private router:Router, private usuarioService: UsuarioService) { }
  ...
  cargarArqueos() {
    ...
    this.arqueoService.getArqueos(loginToken).subscribe(data => {
      ...
      for (const arqueo of this.arqueos) {
        ...
        if (this.comprobantes[contador].length > 0) {
          for (const elemento of this.comprobantes[contador]) {
            ...
            if (elemento.comprobante != 'RETIRO' && elemento.comprobante != 'TARJETA' && elemento.comprobante != 'CHEQUE' && elemento.comprobante != 'DEPOSITO') {
              this.montoGastos[contador] = this.montoGastos[contador] + Number(elemento.monto);
            }

            if (elemento.comprobante == 'RETIRO') {
              this.montoRetiro[contador] = this.montoRetiro[contador] + Number(elemento.monto);
            }

            if (elemento.comprobante == 'TARJETA') {
              this.montoTarjeta[contador] = this.montoTarjeta[contador] + Number(elemento.monto);
            }

            if (elemento.comprobante == 'DEPOSITO') {
              this.montoDeposito[contador] = this.montoDeposito[contador] + Number(elemento.monto);
            }
  ...
  anularArqueo(id) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.arqueoService.anularArqueo(loginToken, id).subscribe(resp => {
